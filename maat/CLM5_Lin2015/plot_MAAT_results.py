...
df = pd.read_csv(
    f"{results_DIR}/out_LinInputs_CLM5.csv",
...
# *** Plot Anet vs CO2_conc comparing MAAT-CLM5 with Lin2015 ***

fig, ax = plt.subplots(figsize=(5, 2.7), tight_layout=True)  # , layout='constrained')
ax.plot(
    CO2_conc, Anet, ".", label="Anet vs CO2_conc (MAAT CLM5)"
...
# *** Plot Anet vs Leaf_temp comparing MAAT-CLM5 with Lin2015 ***

fig, ax = plt.subplots(figsize=(5, 2.7), tight_layout=True)  # , layout='constrained')
ax.plot(
    leaf_temp, Anet, ".", label="Anet vs Leaf Temp (MAAT CLM5)"
)  # Plot some data on the axes.
ax.plot(
...
# *** Plot Simulated vs Observed Anet comparing MAAT-CLM5 with Lin2015 ***

fig, ax = plt.subplots(figsize=(5, 2.7), tight_layout=True)  # , layout='constrained')
ax.plot(
    Anet_Lin, Anet, ".", label="Simulated vs Observed (MAAT CLM5 vs Lin 2015)"
)  # Plot some data on the axes.
...
ax.set_title("CLM5 in MAAT")  # Add a title to the axes.
ax.legend()  # Add a legend.
